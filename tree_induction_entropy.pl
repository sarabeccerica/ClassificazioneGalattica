...
lancia_apprendi(Classe) :-
    ...
    write('File di output: '), writeln(NomeFile),
    (tell(NomeFile) ->
        write('File di output aperto: '), writeln(NomeFile);
        writeln('Errore: impossibile aprire il file di output'), fail),
    write('Debug: Apertura del file completata'), nl, % Debug
    write('Sto per chiamare apprendi per la classe: '), writeln(Classe),
    findall(e(C, O), e(C, O), Esempi),
    write('Esempi trovati: '), writeln(Esempi),
    (apprendi(Esempi, Classe, Descrizione) ->
        write('Descrizione appresa: '), writeln(Descrizione);
        writeln('Errore: apprendi ha fallito')),
    told.
